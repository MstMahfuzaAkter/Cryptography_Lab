...
def get_text(prompt, size):
    """Keeps asking until the text is exactly `size` bytes long."""
    while True:
        s = input(prompt)
...
def pad(data):
    """Fill the last block: add n bytes, each with value n."""
    n = 16 - len(data) % 16
...
def unpad(data):
    ...
    if n < 1 or n > 16 or data[-n:] != bytes([n]) * n:
        raise ValueError("wrong key or wrong ciphertext")
    return data[:-n]


def get_aes_key():
    return get_text("Key (exactly 16 characters): ", 16).encode("utf-8")
...
def aes_menu():
    ...
    while True:
        ...
        if ch == "1":
            text = input("Plaintext (any text): ")
            key = get_aes_key()
            show = input("Show every step? (y/n): ").lower() == "y"
            data = pad(text.encode("utf-8"))
            cipher = b""
            b = 0
            while b < len(data) // 16:              # one block at a time
                print("\n===== Block %d =====" % (b + 1))
                cipher += aes_encrypt_block(data[b * 16:b * 16 + 16], key, show)
                b += 1
            ...
            try:
                ...
                if len(cipher) == 0 or len(cipher) % 16 != 0:
                    raise ValueError("ciphertext must be a multiple of 32 hex digits")
                plain = b""
                b = 0
                while b < len(cipher) // 16:
                    print("\n===== Block %d =====" % (b + 1))
                    plain += aes_decrypt_block(cipher[b * 16:b * 16 + 16], key, show)
                    b += 1
                print("\n>>> PLAINTEXT :", unpad(plain).decode("utf-8"))
